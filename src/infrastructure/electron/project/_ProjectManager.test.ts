...
function makeMockFileSystem(state = MockFileSystemState()) {
	const fsImpl: Partial<FileSystem.FileSystem> = {
      ...
      onNone: () => Effect.fail( new BadArgument({
        module: "FileSystem",
...
function makeMockProjectManagerCore(recents: ReadonlyArray<ProjectInfo> = []) {
	let store = [...recents];
...
function MockProjectManagerCore(recents: ReadonlyArray<ProjectInfo> = []) {
	return Layer.succeed(ProjectManagerCore, makeMockProjectManagerCore(recents).mock);
}

function TestProjectManagerCore(state = MockFileSystemState(), recents: ReadonlyArray<ProjectInfo> = []) {
	return ProjectManagerLive.pipe(Layer.provide(MockProjectManagerCore(recents))).pipe(
...
describe("ProjectManagerLive", () => {
    ...
    const projectManagerImpl = await Effect.runPromise(ProjectManager.pipe(
      Effect.provide(TestProjectManagerCore({ ...MockFileSystemState(), files: new Map<string, string>([[pkgPath, pkgContent]]) }))
    ));
    ...
    // Rest close function.
    const currentAfterClose    = await Effect.runPromise(Effect.zipRight(projectManagerImpl.close(), projectManagerImpl.getCurrent()));
  ...
  it("should open a project with all fields in package.json", async () => {
    ...
    const project = await Effect.runPromise(ProjectManager.pipe(
      Effect.provide(TestProjectManagerCore({ ...MockFileSystemState(), files: new Map<string, string>([[pkgPath, pkgJson]]) })),
